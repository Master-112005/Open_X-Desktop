...
describe('Dynamic Island Schedule Alerts', function() {
  ...
  it('should render schedule alerts inside the Dynamic Island with inline actions', function() {
    ...
    assert.match(main, /function analyzeDesktopChatIncomingPrompt\(preview = '', senderName = ''\)/);
    assert.match(main, /function speakDesktopChatPrompt\(prompt, metadata = \{\}\)/);
    assert.match(main, /intent: 'desktopChat\.message'/);
    assert.match(main, /kind: 'desktop-chat-reply'/);
    assert.match(main, /label: 'Tell OK'/);
    assert.match(main, /label: 'Open Chat'/);
    assert.match(main, /speakDesktopChatPrompt\(assistantPrompt\.prompt/);
    ...
    assert.match(main, /'desktopChat:quickReply'/);
    ...
    assert.match(preload, /ipcRenderer\.invoke\('desktopChat:quickReply'/);
    assert.match(preload, /ipcRenderer\.invoke\('window:openPeopleChat'/);
    assert.match(preload, /Reply sent in OpenX Chat/);
    assert.match(voiceOverlay, /conversationId: String\(action\?\.conversationId \|\| ''\)/);
    assert.match(voiceOverlay, /\(action\.kind \|\| action\.id\) === 'desktop-chat-reply'/);
  });
});
