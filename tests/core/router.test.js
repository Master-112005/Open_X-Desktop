...
describe('Action Router', function() {
  ...
  it('should route timer reminder and alarm management commands', async function() {
    ...
    const commands = new Map([
      ...
      ['cancel the reminder', 'reminder.cancel'], ['cancle the remindee', 'reminder.cancel'], ['delete all reminders', 'reminder.clear'],
      ['remove all reminders', 'reminder.clear'], ['snooze this reminder for 10 minutes', 'reminder.snooze'], ['snooze the alarm', 'alarm.snooze'],
      ['show my alarms', 'alarm.list'], ['how many alarms today', 'alarm.list'], ['delete all alarms', 'alarm.clear'],
      ['cancel the alarm', 'alarm.cancel'], ['clane the alarm', 'alarm.cancel'], ['remove all alarms', 'alarm.clear'],
  ...
  it('should route send file, folder, and image requests to the phone transfer action', async function() {
    ...
    const phoneOriginResult = await router.process('send me latest screenshot', 'phone', {
      phoneContext: { deviceId: 'phone001', deviceName: 'Galaxy S25' }
    });
    const politePhoneOriginResult = await router.process('please send me report pdf here', 'phone', {
      phoneContext: { deviceId: 'phone001', deviceName: 'Galaxy S25' }
    });
