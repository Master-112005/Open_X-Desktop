...
class VoiceOverlay extends EventEmitter {
  ...
  _normalizeActions(result = {}) {
    ...
    return actions.slice(0, 3).map(action => Object.freeze({
      ...
      conversationId: String(action?.conversationId || '').slice(0, 100),
      text: String(action?.text || '').replace(/\s+/g, ' ').trim().slice(0, 120),
      ...
      if ((action.kind || action.id) === 'desktop-chat-reply') return Boolean(action.conversationId && action.text);
