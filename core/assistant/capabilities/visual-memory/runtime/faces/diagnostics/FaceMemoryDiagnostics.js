...
class FaceMemoryDiagnostics {
  ...
  record(event, details = {}) {
    ...
    this.logger?.debug?.('[FaceMemory]', entry);
    return entry;
  }
  ...
  error(event, error) {
    ...
    this.logger?.error?.('[FaceMemory]', serialized);
    return serialized;
  }

  summary() {
    return { uptimeMs: Date.now() - this.startedAt, events: this.events.length, errors: this.errors, recent: this.events.slice(-25) };
  }
}

module.exports = FaceMemoryDiagnostics;
